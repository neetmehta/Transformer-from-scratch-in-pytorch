...
def greedy_decode(src, src_mask, model, tokenizer, config, device):
    ...
    for _ in range(config.seq_len):
        ...
        cross_attn_mask = torch.ones((1,pred.shape[1]), dtype=bool, device=device).unsqueeze(0).unsqueeze(3) | src_mask.unsqueeze(2)
        ...
        pred = torch.cat((pred, torch.tensor([[next_word]], dtype=torch.long, device=device)), dim=-1)
